...
__GET_AUTO_REQUESTS_LIMIT__  = 'ljsfreq.py --query --status=%s --grid-name=%s --facility=%s --max-records=%d --offset=%d'
__GET_RELEASE_DATA__         = {
                                1: 'ljsfreq.py --queryrel --rel="%s" --create-conf="%s"'
                               ,2: 'ljsfinfo.py --queryrel="%s" --create-conf="%s" --cache --cache-timeout 3600'
                               }
...
__GET_TASK_TOKENS__          = 'ljsfinfo.py --show-task-tokens --cache --cache-timeout 86400'
__SKIP__                     = 0
__GETOUTPUT_RATIO__          = 15
...
class AutoInstall:
    ...
    mode           = "production"
    adminEmail     = None
    maxRequestHandlers  = 1
    requestHandlerTID   = []
    #requestHandlerTID   = None
    autoabortHandlerTID = None
    getOutputHandlerTID = None
    ...
    def start (self, maxproxy, timeleft, interval, compress, dirs, sitechecks, wmsns, wmslb, wmsld,wmswmproxy, wmsmyproxy, mode, adminEmail, gridName, facility, maxchunk, requestThreads, randomize, max_inst_jobs):
        auth_type = ['grid',None]
        self.mode = mode
        self.adminEmail = adminEmail
        self.maxRequestHandlers = requestThreads
        self.randomize = randomize
        self.max_inst_jobs = max_inst_jobs
        utils=ljsfUtils()
        ...
        while not self.shutdown:
            self.status["MAIN"] = 'checking proxy'
            ...
            if (self.proxyTimeLeft > timeleft):
                ...
                self.status["MAIN"] = 'renewing proxy'
            ...
            if (len(self.requestHandlerTID) == 0):
                for tid in range(0,self.maxRequestHandlers):
                    self.status["REQUEST"].append("starting")
                    self.requestHandlerTID.append(thread.start_new_thread(self.requestHandler,(timeleft, interval, sitechecks, wmsns, wmslb, wmsld,wmswmproxy, wmsmyproxy, gridName, facility, maxchunk, tid)))
            #if (not self.requestHandlerTID):
            #    self.requestHandlerTID = thread.start_new_thread(self.requestHandler,(timeleft, interval, sitechecks, wmsns, wmslb, wmsld,wmswmproxy, wmsmyproxy, gridName, facility, maxchunk))
            if (not self.autoabortHandlerTID):
                self.autoabortHandlerTID = thread.start_new_thread(self.autoabortHandler,(timeleft, interval, wmsns, wmslb, wmsld, wmswmproxy, wmsmyproxy, gridName, facility, maxchunk))
            if (not self.getOutputHandlerTID):
                self.getOutputHandlerTID = thread.start_new_thread(self.getOutputHandler,(timeleft, interval, compress))

            if (self.proxyTimeLeft > interval):
                self.status["MAIN"] = 'waiting'
                ...
                self.status["MAIN"] = 'exiting'
                msgOut("Proxy time left is %d s." % self.proxyTimeLeft)
                msgOut("Exiting AutoInstall agent...")
                self.shutdown = True
        for tid in self.requestHandlerTID:
            try: tid.join()
            except: msgOut("Cannot join TID %s" % tid)
        #if (self.requestHandlerTID):   self.requestHandlerTID.join()
        if (self.autoabortHandlerTID): self.autoabortHandlerTID.join()
        if (self.getOutputHandlerTID): self.getOutputHandlerTID.join()
        if (self.debug): msgOut("Exiting AutoInstall agent...")

    # Start the request handler
    def requestHandler (self, timeleft, interval, sitechecks, wmsns, wmslb, wmsld,wmswmproxy, wmsmyproxy, gridName, facility, maxchunk, tid=0):
        utils=ljsfUtils()
        # Start the infinite loop
        if (self.debug): msgOut("Starting loop")
        while not self.shutdown:
            try:
                # Count the pending jobs
                #self.countPendingJobs()
                #self.status["REQUEST"] = 'checking proxy'
                #self.proxyTimeLeft = utils.getProxyLifetime()
                if (maxchunk): maxskip = int(math.ceil(float(maxchunk)/self.maxRequestHandlers))
                else:          maxskip = maxchunk
                if (self.proxyTimeLeft > timeleft):
                    if not self.drain:
                        ...
                        if (instance_jobs <= self.max_inst_jobs):
                            ...
                            self.status["REQUEST"][tid] = 'getting autorun requests'
                            if (self.debug): msgOut("Getting autorun requests for grid %s" % gridName)
                            requests = self.getRequests(wmsns, wmslb, wmsld, wmswmproxy, wmsmyproxy, 'autorun', gridName, facility, maxchunk, tid)
                            if (self.debug): msgOut("Got %d autorun requests for grid type '%s' [%s] <%s>" % (len(requests),gridName,facility,tid))
                            reqindx = 0
                            skip = 0
                            for request in requests:
                                reqindx+=1
                                self.status["REQUEST"][tid] = 'processing autorun requests (%d/%d) [%d]' % (reqindx,self.numrequests,instance_jobs)
                                if (skip > 0):
                                    skip -= 1
                                else:
                                    if (self.debug):
                                        msgOut("Processing autorun request for %s %s %s" % (request['type'], request['rel'], request['ce']))
                                    # Define and submit the jobs
                                    s = self.submitJob(sitechecks, request)
                                    if (s==0):
                                        self.changeRequestStatus(request,'accepted')
                                    else:
                                        if (self.randomize): skip = int(random.random() * maxskip)
                                        msgOut("[%s] Job not submitted. Skipping %d requests" % (tid,skip))
                                if (self.shutdown): break
                        else:
                            self.status["REQUEST"][tid] = 'draining (%d jobs)' % instance_jobs
                ...
                if (self.debug): msgOut("End of loop")
                #self.proxyTimeLeft = utils.getProxyLifetime()
                if (self.proxyTimeLeft > interval):
                    self.status["REQUEST"][tid] = 'waiting'
                    if (self.debug): msgOut("Sleeping %d seconds..." % interval)
                    intervals = int(interval/0.5)
                    for num in range(0,intervals):
                        time.sleep(0.5)
                    ...
                    self.status["REQUEST"][tid] = 'exiting'
                    msgOut("Exiting AutoInstall request handler...")
                    self.shutdown = True
            except:
                pass
        if (self.debug): msgOut("Exiting AutoInstall request handler...")

    # Start the autoabort handler
    def autoabortHandler (self, timeleft, interval, wmsns, wmslb, wmsld, wmswmproxy, wmsmyproxy, gridName, facility, maxchunk):
        utils=ljsfUtils()
        # Start the infinite loop
        while not self.shutdown:
            #self.proxyTimeLeft = utils.getProxyLifetime()
            if (self.proxyTimeLeft > timeleft):
                ...
                self.status["ABORT"] = 'getting autoabort requests'
                requests = self.getRequests(wmsns, wmslb, wmsld, wmswmproxy, wmsmyproxy, 'autoabort', gridName, facility, maxchunk)
                if (self.debug): msgOut("Got %d autoabort requests for grid type '%s' [%s]" % (len(requests),gridName,facility))
                reqindx = 0
                for request in requests:
                    reqindx+=1
                    self.status["ABORT"] = 'processing autoabort requests (%d/%d)' % (reqindx,self.numrequests)
            ...
            if (self.proxyTimeLeft > interval):
                ...
                self.status["ABORT"] = 'exiting'
                msgOut("Proxy time left is %d s." % self.proxyTimeLeft)
                msgOut("Exiting AutoInstall autoabort handler...")
                self.shutdown = True
        if (self.debug): msgOut("Exiting AutoInstall autoabort handler...")

    # Output retrieval handler
    def getOutputHandler (self, timeleft, interval, compress):
        utils=ljsfUtils()
        ljsflog = LJSFlog()
        gbbc = gBBc()
        gbbc.compress = True
        ...
        while not self.shutdown:
            ...
            if (self.proxyTimeLeft > timeleft):
                ...
                # Get the job output
                self.status["GETOUTPUT"] = 'getting output'
                self.getOutput(compress,dirs)

            #self.proxyTimeLeft = utils.getProxyLifetime()
            go_interval = interval * __GETOUTPUT_RATIO__
            if (self.proxyTimeLeft > go_interval):
                self.status["GETOUTPUT"] = 'waiting'
                if (self.debug): msgOut("Sleeping %d seconds..." % go_interval)
                intervals = int(go_interval/0.5)
                for num in range(0,intervals):
                    time.sleep(0.5)
                    if self.shutdown: break
            else:
                self.status["GETOUTPUT"] = 'exiting'
                msgOut("Proxy time left is %d s." % self.proxyTimeLeft)
                msgOut("Exiting AutoInstall output retrieval handler...")
                self.shutdown = True
        if (self.debug): msgOut("Exiting AutoInstall output retrieval handler...")

    def getRequests(self, ns, lb, ld, wmproxy, myproxy, reqtype='autorun', gridName='ALL', facility='ALL', maxchunk=None, tid=0):
        requests=[]
        if (maxchunk):
            recmax = int(math.ceil(float(maxchunk)/self.maxRequestHandlers))
            offset = int(tid*recmax)
            cmd = __GET_AUTO_REQUESTS_LIMIT__ % (reqtype,gridName,facility,recmax,offset)
        else:
    ...
    def submitJob(self, sitechecks, request):
        ...
        (s,o) = commands.getstatusoutput(__GET_RELEASE_DATA__[self.protocol] % (request['rel'],tmpconf))
        msgOut (__GET_RELEASE_DATA__[self.protocol] % (request['rel'],tmpconf))
        reldata = o.split("\n")[0].split(",")
    ...
    def show_page(self,code=200):
        ...
        try:
            if (self.path.endswith(".html") or self.path.endswith(".css")):
                ...
                page = page.replace("@STATUS@",agent.status["MAIN"])
                statusTID = 0
                requestStatusMessage = ""
                for requestStatus in agent.status["REQUEST"]:
                    if (statusTID > 0): requestStatusMessage += "<BR>"
                    requestStatusMessage += "ID %d: %s" % (statusTID, requestStatus)
                    statusTID += 1
                page = page.replace("@REQUESTSTATUS@",requestStatusMessage)
                page = page.replace("@ABORTSTATUS@",agent.status["ABORT"])
                page = page.replace("@GETOUTPUTSTATUS@",agent.status["GETOUTPUT"])
...
if __name__ == '__main__':
    ...
    try:
        ...
        if (config.has_section("autoinstall")):
            ...
            if (config.has_option("autoinstall", "maxchunk")):
                maxchunk = int(config.get("autoinstall", "maxchunk"))
            ...
            if (config.has_option("autoinstall", "requestThreads")):
                requestThreads = int(config.get("autoinstall", "requestThreads"))
            if (config.has_option("autoinstall", "randomizeSubmission")):
                randomize = config.getboolean("autoinstall", "randomizeSubmission")
            if (config.has_option("autoinstall", "maxInstanceJobs")):
                max_inst_jobs = int(config.get("autoinstall", "maxInstanceJobs"))
    ...
    agent.start(maxproxy,timeleft,interval,compress,dirs,sitechecks,wmsns,wmslb,wmsld,wmswmproxy,wmsmyproxy,mode,adminEmail,gridName,facility,maxchunk,requestThreads,randomize,max_inst_jobs)
